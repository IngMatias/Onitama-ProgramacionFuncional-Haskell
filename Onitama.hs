...
beginning cards = GameState RedPlayer cards initTable
    where initTable = [
...
activePlayer (GameState player _ _) = player
...
endedGame table
    | owner (pieceAt (0,2) table) /= RedPlayer = True
    | owner (pieceAt (4,2) table) /= BluePlayer = True
